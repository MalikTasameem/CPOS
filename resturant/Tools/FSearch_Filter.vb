...
'            Str = _SearchColumn & "  = '" & Text.Text & "'  or " & _SearchColumn & "  like '%" & words(0) & "%' or " & _SearchColumn & "  like '%" & Text.Text & "' or " & _SearchColumn & "  like '" & Text.Text & "%'"
'        Else
...
'            Dim Dv As DataView
'            Dv = IM_DT.AsDataView
'            Dv.RowFilter = IM_Serach(Text.Text)
...
'        If Not String.IsNullOrWhiteSpace(_Table) Then

'            Try
'                Dim C As New C
'                C.Da = New SqlClient.SqlDataAdapter("Select " & _ID & "," & _Column & " from " & _Table & " Order by " & _OrderByField, C.Con)
'                IM_DT = New DataTable
'                C.Da.Fill(IM_DT)
'                'QuickView.DataSource = C.Dt
'                QuickView.DataSource = IM_DT
'                If Me.QuickView.Columns(0).Visible = True Then Me.QuickView.Columns(0).Visible = False
...
'        'If IM_DT.Rows.Count = 0 Then GetLIst_ALL()
'        Dim results As DataRow() = IM_DT.Select(_ID & "  = '" & ID_ & "'")
'        Set_IM(results(0).Item(_Column), results(0).Item(_ID))
'        RollList()
'        ' End If
