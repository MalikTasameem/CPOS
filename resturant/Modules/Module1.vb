...
Module Module1
    ...
    Function Code128(ByVal TheText As String, ByVal CodeLetter As String, ByVal desc As String, ByVal MPUP As Double, ByVal MPLEFT As Double, ByVal MBUP As Double, ByVal MBLEFT As Double, ByVal width As Double, ByVal hieght As Double) As Image
        ...
        MPUP = ConvertToPixel(MPUP)
        MPLEFT = ConvertToPixel(MPLEFT)
        MBUP = ConvertToPixel(MBUP)
        MBLEFT = ConvertToPixel(MBLEFT)
        ' انشاء صورة عرضها عدد حروف الباينرى المستخدم
        width = ConvertToPixel(width)
        hieght = ConvertToPixel(hieght)
        If Len(Binaryz) <= width - (MBLEFT * 2) Then
            Dim bmp As Bitmap = New Bitmap(width, hieght, System.Drawing.Imaging.PixelFormat.Format24bppRgb)
            Dim z As String ' متغير لمعرفة لون الخط 
            Dim GraphZ As Graphics = Graphics.FromImage(bmp)
            GraphZ.CompositingQuality = CompositingQuality.HighQuality
            Dim RectZ1 As Rectangle = New Rectangle(0, 0, bmp.Width, bmp.Height)
            Dim myBrush1 As Brush = New Drawing.Drawing2D.LinearGradientBrush(RectZ1, Color.White, Color.White, Drawing.Drawing2D.LinearGradientMode.ForwardDiagonal)
            GraphZ.FillRectangle(myBrush1, RectZ1)
            'width = width - (MBLEFT * 2)
            'hieght = hieght - (MBUP * 2)
            Dim size As New Size(TextRenderer.MeasureText(TheText, New Font("arial", 8, FontStyle.Bold)))
            GraphZ.DrawString(TheText, New Font("arial", 8, FontStyle.Bold), New SolidBrush(Color.Black), width / 2 - (size.Width / 2) + (MBLEFT), 0 + MBUP)
            '  رسم خطوط الباركود
            Dim PenZ As Pen
            ...
            For I = 1 To Len(Binaryz)
                ...
                If z = "0" Then
                    ...
                    point1 = New Point(I + ((width / 2) - (Len(Binaryz) / 2)) + MBLEFT, 12 + MBUP)
                    point2 = New Point(I + ((width / 2) - (Len(Binaryz) / 2)) + MBLEFT, hieght - 13 - MBUP)
                    GraphZ.DrawLine(PenZ, point1, point2)
                Else
                    PenZ = New Pen(Color.White, 1)
                    point1 = New Point(I + ((width / 2) - (Len(Binaryz) / 2)), 12 + MBUP)
                    point2 = New Point(I + ((width / 2) - (Len(Binaryz) / 2)), hieght - 13 - MBUP)
                    GraphZ.DrawLine(PenZ, point1, point2)
                End If
            Next
            size = New Size(TextRenderer.MeasureText(desc, New Font("arial", 8, FontStyle.Bold)))
            ' رسم النص المراد ترميزه اسفل الكود
            GraphZ.DrawString(desc, New Font("arial", 8, FontStyle.Bold), New SolidBrush(Color.Black), width / 2 - (size.Width / 2) + (MBLEFT), hieght - 12 - MBUP)
            ' ارجاع الصورة النهائية للدالة
            Code128 = bmp
        Else
            Dim OriginalD As Double = width
            Dim Size As New Size(TextRenderer.MeasureText("الباركود الذي ادخلته لايتسع للحجم المحدد" & vbNewLine & " الرجاء زيادة عرض الباركود", New Font("arial", 8, FontStyle.Bold)))

            Dim bmp As Bitmap = New Bitmap(Size.Width, Size.Height, System.Drawing.Imaging.PixelFormat.Format24bppRgb)
            'Dim z As String ' متغير لمعرفة لون الخط 
            Dim GraphZ As Graphics = Graphics.FromImage(bmp)
            Dim RectZ As Rectangle = New Rectangle(0, 0, bmp.Width, bmp.Height)
            GraphZ.FillRectangle(New Drawing.Drawing2D.LinearGradientBrush(RectZ, Color.White, Color.White, Drawing.Drawing2D.LinearGradientMode.ForwardDiagonal), RectZ)
            'GraphZ.SmoothingMode = Drawing2D.SmoothingMode.HighQuality
            'GraphZ.InterpolationMode = InterpolationMode.HighQualityBicubic
            ' رسم النص المراد ترميزه اسفل الكود
            GraphZ.DrawString("الباركود الذي ادخلته لايتسع للحجم المحدد" & vbNewLine & " الرجاء زيادة عرض الباركود", New Font("arial", 8, FontStyle.Bold), New SolidBrush(Color.Red), 0, 0)
            ' ارجاع الصورة النهائية للدالة
            '  bmp = New Bitmap(bmp, OriginalD, hieght)
            Code128 = bmp
        End If
    End Function
    Function Code128_2(ByVal TheText As String, ByVal CodeLetter As String, ByVal desc As String, ByVal MPUP As Double, ByVal MPLEFT As Double, ByVal MBUP As Double, ByVal MBLEFT As Double, ByVal width As Double, ByVal hieght As Double) As Image
        ' TheText متغير خاص بالنص المراد تشفيره
        ' CodeLetter متغير خاص بالفئة المراد استخدامها
        ...
        If Len(Binaryz) <= width - (MBLEFT * 2) Then
            ...
            For I = 1 To Len(Binaryz)
                ...
                If z = "0" Then
                    ...
                    point1 = New Point(I + ((width / 2) - (Len(Binaryz) / 2)) + MBLEFT, 1 + MBUP)
                    point2 = New Point(I + ((width / 2) - (Len(Binaryz) / 2)) + MBLEFT, hieght - 1 - MBUP)
                    GraphZ.DrawLine(PenZ, point1, point2)
                Else
                    PenZ = New Pen(Color.White, 1)
                    point1 = New Point(I + ((width / 2) - (Len(Binaryz) / 2)), 1 + MBUP)
                    point2 = New Point(I + ((width / 2) - (Len(Binaryz) / 2)), hieght - 1 - MBUP)
                    GraphZ.DrawLine(PenZ, point1, point2)
                End If
            Next
            ' size = New Size(TextRenderer.MeasureText(desc, New Font("arial", 8, FontStyle.Bold)))
    ...
    Function Toc(ByVal Text2CStr As String) As String
        ...
        Toc = NewText
    End Function
    Public Function ConvertToPixel(cm As Double) As Double
        Dim pixel As Double
        'cm = cm + 0.05
        Dim ErrorRate As Double = cm * 0.03947368
        ErrorRate = ErrorRate * 37.795275591
        pixel = cm * 37.795275591
        Return pixel - ErrorRate
    End Function
End Module
