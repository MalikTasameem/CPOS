Public Class Agent_Balance_For_Tree
    ...
    Private Sub AGMetroGrid_MouseDoubleClick(sender As Object, e As MouseEventArgs) Handles EXP_DataGridView.MouseDoubleClick
        ...
        If inp <> "" Then
            query("UPDATE Expenses_Card SET TREE_CODE = '" & inp & "' WHERE Ex_ID = " & EXP_DataGridView.CurrentRow.Cells("T_ID_CL").Value)
            EXP_DataGridView.CurrentRow.Cells(2).Value = inp
            'Load_MAIN_BALANCES()
        End If
    End Sub
    ...
    Private Sub Load_EXP_BALANCES()
        Dim c As New C

        Try
            IM_Dt.Clear()
            Dim s As String
            s = "select [Ex_ID],[Ex_Name],TREE_CODE from [Expenses_Card] "
            c.Da = New SqlClient.SqlDataAdapter(s, c.Con)
            c.Da.Fill(IM_Dt)
    ...
    Private Sub GENERAL_TXT_TextChanged(sender As Object, e As EventArgs) Handles GENERAL_TXT.TextChanged
        ...
        EXP_DataGridView.DataSource = Dv
