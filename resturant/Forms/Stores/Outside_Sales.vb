Public Class Outside_Sales : Inherits System.Windows.Forms.Form
    ...
    Public Sub Get_Last_T_ID()
            Dim C As New C
        Dim S As String = "Select Top 1 T_ID From Agents_Balance_MV Where User_ID = '" & USER_ID & "' AND BsType_ID = 35 AND isDepended = 0 AND isVoid = 0 AND T_ID BETWEEN " & START_ID & " AND " & END_ID & " ORDER BY T_ID DESC"
    ...
    Public Sub SELECT_MAX()
        ...
        Try
            Dim s As String
            s = "SELECT ISNULL(MAX(Outsale_ID),0) + 1 AS N FROM Agents_Balance_MV "
            c.Com = New SqlClient.SqlCommand(s, c.Con)
            c.Con.Open()
            c.Dr = c.Com.ExecuteReader
            If c.Dr.HasRows Then
    ...
    Private Sub Edit_butt_Click(sender As Object, e As EventArgs) Handles Edit_butt.Click
        If Edit_butt.Text = EditState Then
            ...
            Network_Edit_Tracker_insert("تعديل فاتورة داخلية / رقم آلي : " + Bill_ID_Txt.Text + " / المدخل :  " + User_Name_lb.Text, Total_TextBox.Text, 0, 0)
    ...
    Private Sub ADDCatButton_Click(sender As Object, e As EventArgs) Handles ADDCatButton.Click
        ...
        '    '    MsgBox("لا توجد كمية للصنف !!", MsgBoxStyle.Critical, "خطأ فالإدخال")
        '    '    Exit Sub
        '    'End If

        '    'If Convert.ToDouble(Current_QTY.Text) < Convert.ToDouble(QtyTextBox.Text) Then
        '    '    MsgBox("الكمية المخصومة أكبر من كمية المخزن !!", MsgBoxStyle.Critical, "خطأ فالإدخال")
        '    '    Exit Sub
        '    'End If
        ...
        F_Outside_Sales_IM_card = Outside_Sales_IM_card
        F_Outside_Sales_IM_card.T_ID = T_ID
        F_Outside_Sales_IM_card.ShowDialog()
    ...
    Private Sub Insert_NewBill()
        ...
        sqlComm.Parameters("@Outsale_ID").Direction = ParameterDirection.Output
        sqlComm.Parameters("@T_ID").Direction = ParameterDirection.Output

        If SQL_SP_EXEC(sqlComm) = True Then
            'Me.Bill_ID_Txt.Text = sqlComm.Parameters("@Outsale_ID").Value.ToString()
    ...
    '    If SQL_SP_EXEC(sqlComm) = True Then
    '        Network_Edit_Tracker_insert(" الصنف:" & IM_SH_txt.Text & " الوحدة:" & IM_Unit_cm.Text & " العدد:" & QtyTextBox.Text & " السعر:" & IM_Cost_txt.Text, Bill_ID_Txt.Text, 35, 1)
    ...
    Public Sub SELECT_EXP_Cats(T_ID As Integer)
        ...
        With C.Com
            .Connection = C.Con
            .CommandText = "[Outsale_Details_SELECT_Bill]"
            .CommandType = CommandType.StoredProcedure
    ...
    Private Sub Delete_Cat()
        ...
        Dim sqlComm As New SqlClient.SqlCommand
        sqlComm.CommandText = "Outsale_Details_Delete"
        sqlComm.CommandType = CommandType.StoredProcedure
        ...
        If SQL_SP_EXEC(sqlComm) = True Then
            ...
            " العدد:" & AGMetroGrid.CurrentRow.Cells("QTY_CL").Value.ToString & " السعر:" & AGMetroGrid.CurrentRow.Cells("Price_CL").Value.ToString, Bill_ID_Txt.Text, 35, 2)
            SELECT_EXP_Cats(T_ID)
            Update_Total()
    ...
    Public Sub Get_T_ID()
        ...
        Dim C As New C
        Dim S As String = "Select T_ID From Agents_Balance_MV Where Outsale_ID = '" & Convert.ToInt64(Bill_ID_Txt.Text) & "'"
        C.Com = New SqlClient.SqlCommand(S, C.Con)
        C.Con.Open()
        Try
            C.Dr = C.Com.ExecuteReader
    ...
    Public Sub IMTranPrintData()
        ...
        Try
            ...
            If MY_Settings.OutSale_rpt = 0 Then
                pp.rp.Load(Application.StartupPath & "\reports\OutSide_Bill.rpt")
            Else
                pp.rp.Load(Application.StartupPath & "\reports\OutSide_Bill_A5.rpt")
            End If
            ...
            With pp
                ...
                .rp.SetParameterValue(8, " الحساب :  " + Emp_FS.Textt)
                .rp.SetParameterValue(9, Pure_SP_txt.Text)
            End With
    ...
    Private Sub علاضبطاقةالصنفToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles علاضبطاقةالصنفToolStripMenuItem.Click
        ...
        Str_ = F_Outside_Sales.AGMetroGrid.CurrentRow.Cells("EX_Name_CL").Value
