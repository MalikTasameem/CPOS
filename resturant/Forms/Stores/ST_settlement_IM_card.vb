Public Class ST_settlement_IM_card : Inherits System.Windows.Forms.Form
    ...
    Private Sub ADDCatButton_Click(sender As Object, e As EventArgs) Handles ADDCatButton.Click
        ...
        '    For i = 0 To F_Invoice.AGMetroGrid.Rows.Count - 1
        '        If F_Invoice.AGMetroGrid.Rows(i).Cells("EX_ID_CL").Value = IM_ID And F_Invoice.AGMetroGrid.Rows(i).Cells("St_ID_CL").Value = ST_cm.SelectedValue Then

        '            If Valid_Panel.Visible = True Then
        '                If F_Invoice.AGMetroGrid.Rows(i).Cells("D_Valid_CL").Value = D_Valid.Text Then
    ...
    Private Sub Insert_Cat()
        ...
        sqlComm.Parameters.AddWithValue("@Price", Convert.ToDouble(PriceTextBox.Text))
        ...
        If String.IsNullOrWhiteSpace(NewSalePrice_txt.Text) = False Then sqlComm.Parameters.AddWithValue("@NewSale", Convert.ToDouble(NewSalePrice_txt.Text))
        If String.IsNullOrWhiteSpace(NewSaleByOne.Text) = False Then sqlComm.Parameters.AddWithValue("@NewSaleByOne", Convert.ToDouble(NewSaleByOne.Text))
        ...
        sqlComm.Parameters.AddWithValue("@On_Update", F_Invoice.On_Update)
        sqlComm.Parameters.AddWithValue("@Barcode", Barcode_IM)

        sqlComm.Parameters.AddWithValue("@Invoice_Type", F_Invoice.Invoice_Type_cm.SelectedIndex)
        ...
        If SQL_SP_EXEC(sqlComm) = True Then
            ...
            If F_Invoice.On_Update = True Then DependingUpdatedBill(T_ID)

            Network_Edit_Tracker_insert(" الصنف:" + mySearchControl.txtSearch.Text + " الوحدة:" + IM_Unit_cm.Text + " كمية التسوية:" + QtyTextBox.Text + " السعر:" + PriceTextBox.Text + " البيع:" _
                                        + NewSalePrice_txt.Text + " بيع القطعة:" + NewSaleByOne.Text, F_Invoice.Bill_ID_Txt.Text, 9, 1)

            F_Invoice.Pch_Contents_SELECT_Bill()
