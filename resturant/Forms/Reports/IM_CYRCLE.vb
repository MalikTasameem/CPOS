...
Public Class IM_CYRCLE
    ...
    Public Sub Load_Data()
        ...
        Try
            sql = " select DISTINCT Type_ID,[نوع المعاملة] AS Type_Name from IM_FULL_MV_V Order By Type_Name ASC"
    ...
    Public Sub Fill_ALL_IM(IM_ID As Integer)
        ...
        'Main_Query = " select '' AS ' ت ' ,IM_CAT AS ' الفئــة ',GM_ANME AS ' المجموعة ',St_Name AS ' المخزن ',Type_Name AS ' المعاملة ',Ag_name AS ' الحساب ',Date AS ' التاريخ ',Bill_ID AS ' رقم المعاملة '," & _
        '    "BARCODE AS ' باركود ',IM_NAME AS ' إسم الصنف ',U_NAME AS ' الوحدة ',U_Cargo AS ' التعادل ',D_Vaild AS ' الصلاحية ',Q_OUT AS ' الوارد ',Q_IN AS ' الصادر ',Price AS ' السعر ',UserName AS ' المدخل ' from [IM_FULL_MV_V]  "

        Main_Query = "SELECT '' AS ' ت '" & _
            ",[الفئة]" & _
            ",[التصنيف]" & _
            ",[المخزن]" & _
            ",[نوع المعاملة]" & _
        ...
        If AllRecieptsCheckBox.Checked = False Then Main_Query = Main_Query & " AND Type_ID = " & ReceiptTypeComboBox.SelectedValue

        Main_Query = Main_Query & " and CONVERT(Date,[التاريخ]) BETWEEN '" & HOME.DateRange_Flate.D_From.Text & "' AND '" & HOME.DateRange_Flate.D_To.Text & "' "


        Main_Query = Main_Query & " ORDER BY [التاريخ] ASC "
    ...
    Private Sub PrintButton_Click(sender As Object, e As EventArgs) Handles PrintButton.Click
        ...
        F.PRINT_PDF_List(gridv, CheckedListBox1, " تقريــر دورة صنـــف ", 1)
    ...
    Public Sub RECOUNT_IM_ALL_QTY()
        ...
        If SQL_SP_EXEC(c.Com) = True Then
            Network_Edit_Tracker_insert(" تدوير كميات الأصناف بمخزن : " & ST_cm.Text & "  من شاشة كشف المخازن ", 0, 23, 3)
            Me.Cursor = Cursors.Default
            Fill_ALL_IM(0)
            Make_Hints()
    ...
    Public Sub RECOUNT_IM_ALL_COST()
        ...
        If SQL_SP_EXEC(c.Com) = True Then
            Network_Edit_Tracker_insert(" تدوير متوسط تكلفة الأصناف من شاشة كشف المخازن   : ", 0, 23, 3)
            Me.Cursor = Cursors.Default
            Fill_ALL_IM(0)
            Make_Hints()
            MsgBox("تم التدوير", MsgBoxStyle.Information, "")
