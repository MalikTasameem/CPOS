Public Class Frm_auto_IM_card : Inherits System.Windows.Forms.Form
        ...
        Try
            ...
            Using cn As New SqlClient.SqlConnection(MY_Settings.SqlConStr)
                ...
                Using unitsCmd As New SqlClient.SqlCommand(
                    "select U_ID,U_Name from IM_Menu_Units_V WHERE IM_ID = @IM_ID Order By U_Cargo Desc",
                    cn
                )
    ...
    Private Sub ApplySelectedUnitData(cn As SqlClient.SqlConnection)
        ...
        Dim newCost As Double = CalcFrmCost(cn)
        ...
        If U_Cargo > 1 Then
            ...
            CostByOne.Text = newCost.ToString()
        Else
        ...
        CalcAvgCost()
        PriceTextBox.Text = (newCost * U_Cargo).ToString()

    End Sub

    Private Function CalcFrmCost(cn As SqlClient.SqlConnection) As Double

        Using costCmd As New SqlClient.SqlCommand(
            "SELECT ISNULL(SUM(CostVQty),0) AS NewCost FROM IM_Formating_Menu_V WHERE IM_ID = @IM_ID",
            cn
        )

            costCmd.Parameters.AddWithValue("@IM_ID", IM_ID)
            Return Convert.ToDouble(costCmd.ExecuteScalar())
