...
Public Class users
    ...
    Private DisplayedTreePermissions As New List(Of TreeMainPermissions.PermissionDef)
    Private CurrentAllowedTreePermissionKeys As New HashSet(Of String)(StringComparer.OrdinalIgnoreCase)
    ...
    Public Sub clear_Text()
        ...
        ID_u = 0
        isAdmin = False
        CurrentAllowedTreePermissionKeys.Clear()
        EditUserButton.Enabled = False
        ChangePassButton.Enabled = False
        DeleteUserButton.Enabled = False
        If TreePermissionsGroupBox IsNot Nothing Then TreePermissionsGroupBox.Enabled = False
        If TreePermissionsList IsNot Nothing Then SetAllTreePermissions(False)
    End Sub
    ...
    Private Sub LoadTreePermissionList()
        TreePermissionsList.Items.Clear()
        DisplayedTreePermissions = GetDisplayedTreePermissions()

        For Each p In DisplayedTreePermissions
            TreePermissionsList.Items.Add(p.GroupName & " - " & p.Title)
        Next

        TreePermissionsGroupBox.Enabled = False
    End Sub

    Private Function GetDisplayedTreePermissions() As List(Of TreeMainPermissions.PermissionDef)
        Dim result As New List(Of TreeMainPermissions.PermissionDef)

        For Each p In TreeMainPermissions.AllPermissions
            If MY_Settings.Use_State_Budget OrElse Not IsBudgetPermission(p.Key) Then
                result.Add(p)
            End If
        Next
    ...
    Private Function IsBudgetPermission(permissionKey As String) As Boolean
        Return Not String.IsNullOrWhiteSpace(permissionKey) AndAlso
               permissionKey.StartsWith("TREE.BUDGET_", StringComparison.OrdinalIgnoreCase)
    End Function

    Private Sub SetAllTreePermissions(checkedValue As Boolean)
        For i As Integer = 0 To TreePermissionsList.Items.Count - 1
            TreePermissionsList.SetItemChecked(i, checkedValue)
        Next
    End Sub

    Private Sub LoadTreePermissionsForSelectedUser()
        If ID_u <= 0 Then
            SetAllTreePermissions(False)
            TreePermissionsGroupBox.Enabled = False
            Exit Sub
        End If

        CurrentAllowedTreePermissionKeys = TreeMainPermissions.LoadAllowedPermissions(ID_u, isAdmin)
        Dim permissions = DisplayedTreePermissions

        For i As Integer = 0 To permissions.Count - 1
            TreePermissionsList.SetItemChecked(i, CurrentAllowedTreePermissionKeys.Contains(permissions(i).Key))
        Next

        TreePermissionsGroupBox.Enabled = (FieldsPanel.Enabled AndAlso Not isAdmin)
    End Sub

    Private Function SelectedTreePermissionKeys() As List(Of String)
        Dim result As New HashSet(Of String)(StringComparer.OrdinalIgnoreCase)
        Dim permissions = DisplayedTreePermissions

        For Each key In CurrentAllowedTreePermissionKeys
            If IsBudgetPermission(key) AndAlso Not MY_Settings.Use_State_Budget Then
                result.Add(key)
            End If
        Next

        For i As Integer = 0 To permissions.Count - 1
            If TreePermissionsList.GetItemChecked(i) Then result.Add(permissions(i).Key)
        Next

        Return New List(Of String)(result)
    End Function

    Private Sub SaveTreePermissionsForCurrentUser()
        If ID_u <= 0 OrElse isAdmin Then Exit Sub
        TreeMainPermissions.SaveUserPermissions(ID_u, SelectedTreePermissionKeys())
    End Sub
...
ORDER BY user_id DESC;", con)
    ...
    Public Sub User_Insert()
        ...
        If SQL_SP_EXEC(c.Com) = True Then
            ID_u = GetUserIdByName(NameUserTextBox.Text)
            SaveTreePermissionsForCurrentUser()
    ...
    Public Sub User_Update()
        ...
        With c.Com
            ...
            If Default_AG_cm.SelectedValue > 0 Then .Parameters.AddWithValue("@AG_ID", Default_AG_cm.SelectedValue)
            .Parameters.AddWithValue("@is_Allow", Is_Allow_CB.Checked)
            .Parameters.AddWithValue("@Tr_ID", 1) 'Treasury_ComboBox.SelectedValue
    ...
    Public Sub USER_Reader_in_Text()
        ...
        AddUserButton.Enabled = False
        LoadTreePermissionsForSelectedUser()
        TreePermissionsGroupBox.Enabled = False
    ...
    Private Sub NewUserButton_Click(sender As Object, e As EventArgs) Handles NewUserButton.Click
        ...
        TreePermissionsGroupBox.Enabled = True
        SetAllTreePermissions(False)
    End Sub
    ...
    Private Sub EditUserButton_Click(sender As Object, e As EventArgs) Handles EditUserButton.Click
        ...
        If EditUserButton.Text = "تعديل" Then
            ...
            FieldsPanel.Enabled = True
            TreePermissionsGroupBox.Enabled = Not isAdmin
            If isAdmin Then SetAllTreePermissions(True)
        Else
    ...
    Private Sub SelectAllTreePermissionsButton_Click(sender As Object, e As EventArgs) Handles SelectAllTreePermissionsButton.Click
        SetAllTreePermissions(True)
    End Sub

    Private Sub ClearTreePermissionsButton_Click(sender As Object, e As EventArgs) Handles ClearTreePermissionsButton.Click
        SetAllTreePermissions(False)
    End Sub
