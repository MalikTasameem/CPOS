Public Class Cities
    ...
    Public Form_Name, Form_Name_Arabic As String
    Public F_ID, F_Name, F_DETAILS, F_DETAILS_TABLE As String
    Dim IM_DT As New DataTable
    Public just_AdminEdit As Boolean = False, WithOne As Boolean = False
    ' Public user_Clinic As New user

    Private Sub NewEmpButton_Click(sender As Object, e As EventArgs) Handles NewSButton.Click

        'If user_Clinic.U_add = 1 Then
    ...
    Public Sub Store_Insert()
        ...
        sqlComm.CommandText = Form_Name & "Proc"
        sqlComm.CommandType = CommandType.StoredProcedure
        sqlComm.Parameters.AddWithValue("@Process", 1)
    ...
    Private Sub STORES_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        'If Not user_Clinic.Open_EntryData Then
        '    MessageBox.Show("ليس لديك الصلاحيات ", "خطأ", MessageBoxButtons.OK, MessageBoxIcon.Error)
        '    Me.Close()
        'End If
    ...
    Public Sub Load_StoreData()
        ...
        Try
            Dim s As String
            s = "select " & F_ID & "," & F_Name & " from " & Form_Name & "  WHERE " & F_ID & " > 1 ORDER BY " & F_Name & " ASC "
            If WithOne Then s = "select " & F_ID & "," & F_Name & " from " & Form_Name & " ORDER BY " & F_Name & " ASC "
    ...
    Public Sub Store_Update()
        ...
        sqlComm.Parameters.AddWithValue("@Process", 3)
    ...
    Private Sub DeleteSButton_Click(sender As Object, e As EventArgs) Handles DeleteSButton.Click
        ...
        '    Exit Sub
        'End If
        'If user_Clinic.U_delete = 0 Then
        '    MessageBox.Show("ليس لديك الصلاحيات", "خطأ", MessageBoxButtons.OK, MessageBoxIcon.Error)
        'Else

        If GET_ID(F_DETAILS, F_DETAILS_TABLE, S_ID) = 1 Then
    ...
    Private Function GET_ID(COLUMN As String, TABLE As String, ID As Integer)
        Dim C = New C
        Try
            Dim s As String
    ...
    Public Sub S_Delete()
        ...
        sqlComm.CommandText = Form_Name & "Proc"
        sqlComm.CommandType = CommandType.StoredProcedure
        sqlComm.Parameters.AddWithValue("@Process", 2)
        sqlComm.Parameters.AddWithValue("@" & F_ID, S_ID)
