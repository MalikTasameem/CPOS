...
Public Class NotificationForm
    ...
    Private Shared notifications As New List(Of NotificationForm)
    Private lblTitle As Label
    Private lblMessage As Label
    Private btnClose As Button
    Private animationTimer As Timer
    Private autoCloseTimer As Timer   ' <-- مؤقت الإغلاق التلقائي
    Private isClosing As Boolean = False
    Private position As String = "top"

    Public Sub New(title As String, message As String, position As String, Optional ByVal is_Error As Boolean = False)
        ...
        If is_Error = True Then Me.BackColor = Color.DarkRed
        Me.Opacity = 0.9
        Me.Width = 300
        Me.Height = 80
        ...
        lblMessage.Font = New Font("Tahoma", 12, FontStyle.Regular)
        ...
        If position = "top" Then
            Me.Top = -Me.Height - (totalNotifications * (Me.Height + offset))
        Else
            Me.Top = screenBounds.Height + (totalNotifications * (Me.Height + offset))
        End If
        ...
        ' مؤقت الإغلاق التلقائي (5 ثواني)
        autoCloseTimer = New Timer()
        autoCloseTimer.Interval = 5000 ' 5 ثواني
        AddHandler autoCloseTimer.Tick, AddressOf AutoClose_Tick
    End Sub
    ...
    Public Sub ShowNotification()
        ...
        Me.Show()
        autoCloseTimer.Start() ' تشغيل مؤقت الإغلاق التلقائي
    End Sub
    ...
    Private Sub CloseNotification(sender As Object, e As EventArgs)
        autoCloseTimer.Stop()   ' ⬅ إيقاف المؤقت إذا ضغط المستخدم قبل انتهاء الـ 5 ثواني
        isClosing = True
        animationTimer.Start()
    End Sub

    ' حدث مؤقت الإغلاق التلقائي
    Private Sub AutoClose_Tick(sender As Object, e As EventArgs)
        autoCloseTimer.Stop()
        CloseNotification(Nothing, EventArgs.Empty)
    End Sub
