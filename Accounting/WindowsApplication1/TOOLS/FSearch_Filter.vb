...
Public Class FSearch_Filter
    ...
    Public is_Dt_Filter As Boolean = False
    Public Dt_Full_Data As New DataTable
    Public pnlFocusHolder As New Panel
    ...
    'Private _originalLocation As Point
    'Private _originalSize As Size
    'Private _isExpanded As Boolean = False

    'Private _suppressSearchFocus As Boolean = False

    'Public Sub Expand()
    '    If _isExpanded Then Exit Sub

    '    _originalParent = Me.Parent
    '    _originalLocation = Me.Location
    ...
    '    ' حساب الموقع الحقيقي
    '    Dim screenPos As Point = _originalParent.PointToScreen(_originalLocation)
    '    Dim formPos As Point = frm.PointToClient(screenPos)

    '    ' نقل الكنترول للفورم
    '    Me.Parent = frm

    '    ' ⬅️ تثبيت X و Y
    '    Me.Location = New Point(formPos.X, formPos.Y)

    '    Me.BringToFront()

    '    ' ⬇️ التوسع للأسفل فقط
    '    Me.Width = _originalSize.Width     ' ثابت
    '    Me.Height = 300                     ' ارتفاع القائمة المنسدلة
    ...
    'Public Sub Collapse()
    '    If Not _isExpanded Then Exit Sub

    '    Me.Parent = _originalParent
    '    Me.Location = _originalLocation
    ...
    Public Sub New()
        ...
        cancelSearchButton.TextAlign = ContentAlignment.MiddleCenter
        ...
        pnlFocusHolder.TabStop = False
        pnlFocusHolder.Visible = False
    ...
    Private Sub CellContentDoubleClick(sender As Object, e As EventArgs)
        Try
            If QuickView.SelectedRows.Count > 0 Then
                ...
                'Clickk()
                'Text.Focus()
                Set_IM(QuickView.SelectedRows(0).Cells(_Column).Value.ToString, QuickView.SelectedRows(0).Cells(_ID).Value.ToString)
                ShowFlag = False
                RollList()
            End If
        Catch ex As Exception
    ...
    Private Sub RollList()
        Try
            If Me.Height > (_ListSize + Text.Height) Then Me.Height -= _ListSize
            QuickView.Size = New Size(Me.Width - 2, _ListSize)
    ...
    Public Sub GetLIst_ALL()
        Try

            Dt = New DataTable
            If is_Dt_Filter = False Then
                ...
                C.Da.Fill(Dt)
            Else
                Dt = Dt_Full_Data.Copy()
            End If

            QuickView.DataSource = Dt

            For Each col As DataGridViewColumn In QuickView.Columns
                If col.Name <> _Column Then
                    col.Visible = False
                Else
                    col.Visible = True
                End If
